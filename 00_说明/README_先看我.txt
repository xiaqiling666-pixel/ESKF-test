...
2026-04-28 收口更新
当前项目已经完成“最低完整离线 ESKF 多源融合系统”的初步搭建。
这里的“完成”指：
数据读取、初始化、IMU predict、GNSS/baro/mag update、fusion_output.csv、metrics、退化记录、配置校验和主线验收测试已经形成闭环。

这里的“未完成”指：
还没有完成真实飞行日志的系统验证、论文主贡献收缩、正式实验对比和工程化参数调试。

当前项目状态应理解为：
最低完整闭环基本完成，
进入真实数据调试、配置收敛和阶段收口阶段。

当前项目状态
截至 2026-04-28，这个项目已经不是最初的教学 demo，最低完整闭环已经基本搭建完成，但还不是完成真实数据验证和论文方法收口的定型项目。
当前已经具备：
...
4. 当前已有固定 GNSS 杆臂补偿，但还没有把杆臂作为状态在线估计，也还没有外参、时间偏移、尺度因子等更完整状态。
